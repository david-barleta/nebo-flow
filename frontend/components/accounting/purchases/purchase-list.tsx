...
interface PurchaseListProps {
  purchases: PurchaseWithStakeholder[];
  searchQuery: string;
...
export default function PurchaseList({
  ...
  const statusFiltered = useMemo(() => {
    ...
    return purchases.filter((s) => s.status === statusFilter);
  ...
  const searchFiltered = useMemo(() => {
    ...
    return statusFiltered.filter((s) => {
      const stakeholderName = s.stakeholder
        ...
        : "Walk-in";
      const pvNum = s.purchase_voucher_number || s.document_number;
      return (
        pvNum.toLowerCase().includes(q) ||
  ...
  const sorted = useMemo(() => {
    ...
    arr.sort((a, b) => {
      ...
      if (sortField === "document_number") {
        const aNum = a.purchase_voucher_number || a.document_number;
        const bNum = b.purchase_voucher_number || b.document_number;
  ...
  if (sorted.length === 0) {
    return (
      <div className="rounded-xl border border-gray-200 bg-white flex items-center justify-center py-16">
        <p className="text-sm text-gray-500">No purchases match your filters.</p>
      </div>
    );
  }
        ...
        <table className="w-full">
          <thead className="bg-gray-50 border-b border-gray-200">
            <tr>
              <th className="px-6 py-3 text-left text-xs font-semibold text-gray-900">
                ...
                >
                  PV No. <SortIcon field="document_number" />
                </button>
              </th>
              <th className="px-6 py-3 text-left text-xs font-semibold text-gray-900">
                <button
          ...
          <tbody>
            {displayed.map((purchase) => (
              <PurchaseRow key={purchase.id} purchase={purchase} onClick={onClick} />
            ))}
          </tbody>
        </table>
      </div>
