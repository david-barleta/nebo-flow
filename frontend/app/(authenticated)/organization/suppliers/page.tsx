...
const defaultBreadcrumbs = [{ label: "Organization" }, { label: "Suppliers" }];
...
export default function SuppliersPage() {
  ...
  const [pageTitle, setPageTitle] = useState("Suppliers");
  ...
  const handleToggleActive = (s: Stakeholder) => {
    ...
    if (s.is_active) {
      // Deactivate
      // TODO: Check for outstanding payables and show warning
      setConfirmDialog({
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Empty state */}
      {isEmpty && <StakeholderEmptyState mode="supplier" onAdd={handleAdd} />}
